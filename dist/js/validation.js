var chatmanagername = document.getElementById('chatmanagername');         // переменная chatmanagername - доступ к input name формы chatmanager "напиши мне я онлайн"
var chatmanagermessage = document.getElementById('chatmanagermessage');   // chatmanagermessage - доступ к полю "введите сообщение" формы chatmanager
var btnsend = document.getElementById('btnsend');                         // btnsend - кнопка "отправить" на форме chatmanager "напиши мне я онлайн"
var chatoffname = document.getElementById('chatoffname');                 // chatoffname - поле name формы chatoff
var chatoffemail = document.getElementById('chatoffemail');               // chatoffemail - поле email формы chatoff
var chatoffmessage = document.getElementById('chatoffmessage');           // chatoffmessage - поле message формы chatoff
var chatoffsend = document.getElementById('chatoffsend');                 // chatoffsend - кнопка "отправить" на форме chatoff


function solidObj (e) {                                                   // ф-ия с 2мя объектами для валидации. Каждый объект есть набор полей конкретной формы
	...
	} else if (e.target.getAttribute('id') == 'chatoffsend') {
		var chatoffObj = {
...
function validFields (obj) {
	...
	var arr_err = [];                                                       // пустой массив для проверки, если не заполнено какое либо поле, то отправка данных
	                                                                        // |-> на сервер не производится
	for (var key in obj) {                                                  // перебираем все ключи и значения объекта
		if (!obj[key].value) {                                                // если поле пустое
			...
			if (valid_mail) {  // если true
				obj[key] = obj[key].value;                                        // в данное поле мы записываем не true а значение, чтобы потом передать это значение
				                                                                  // |-> в json файле
	...
	if (arr_err.length == 0) {                                              // если обхект пуст, значит все поля заполнены и можно отправлять данные на сервер
		...
		var url = '../php/manager.php';
		var result = 'data_form=' + JSON.stringify(obj);
		...
		xhr.addEventListener('readystatechange', function () {
			if (xhr.readyState == 4 && xhr.status == 200) {
				...
				console.log(JSON.parse(object));                                  // консолим для себя чтоб посмотреть содержимое файла
			}
		});
		xhr.send(result);
	}
